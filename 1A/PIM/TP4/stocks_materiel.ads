...
package Stocks_Materiel is
	...
	type T_State is (READY, MALFUNCTIONNING);

    type TR_Mat is record
    	Nature : T_Nature ;
    	State : T_State ;
    	BYear : Integer ;
    	Serial : Integer ;
	...
	type T_Mat is access TR_Mat ;
	
	type T_Stock is array (1 .. 10) of T_Mat ;
	...
	begin
		...
		New_Mat.Serial := Numero_Serie ;
		New_Mat.Nature := Nature ;
		New_Mat.BYear := Annee_Achat ;
		New_Mat.State := READY ;
		
		Stock (Stock'Last +1) := New_Mat ;
