...
SenderWrite2 == /\ SenderLive
                ...
                /\ SenderState' = Blocked
                /\ NotifyRead' = TRUE
                /\ Len(Buffer) >= BufferSize (* Le buffer est complet *)
                /\ UNCHANGED << Sent, msg, Got, SenderLive, ReceiverLive, Buffer, ReceiverState, ReceiverIT, NotifyWrite, SenderIT >>
...
SenderWriteNext1 == /\ SenderLive
                    /\ SenderState = AfterWriting
                    /\ SenderState' = Idle
                    /\ SenderIT' = NotifyWrite
                    /\ NotifyWrite' = FALSE
                    /\ Len(msg) <= 0 (* Le message a envoyer est vide *)
                    /\ UNCHANGED << Sent, Got, msg, SenderLive, ReceiverLive, Buffer, ReceiverState, ReceiverIT, NotifyRead >>
...
SenderWriteNext2 == /\ SenderLive
                    ...
                    /\ SenderIT' = NotifyWrite
                    /\ NotifyWrite' = FALSE
                    /\ Len(msg) >= 0 (* Le message a envoyer n'est pas vide *)
                    /\ UNCHANGED << Sent, Got, msg, SenderLive, ReceiverLive, Buffer, ReceiverState, ReceiverIT, NotifyRead >>
...
SenderUnblock1 ==  /\ SenderLive
                   ...
                   /\ SenderState = Blocked
                   /\ SenderState' = Writing
                   /\ UNCHANGED << Sent, msg, Got, SenderLive, ReceiverLive, Buffer, ReceiverState, NotifyWrite, NotifyRead, SenderIT >>
...
SenderUnblock2 ==  /\ \neg ReceiverLive
                   ...
                   /\ SenderState = Blocked
                   /\ SenderState' = Done
                   /\ UNCHANGED << Sent, msg, Got, SenderLive, ReceiverLive, Buffer, ReceiverState, NotifyWrite, ReceiverIT, NotifyRead, SenderIT >>
...
ReceiverIdle2 == /\ ReceiverLive
                 ...
                 /\ Len(Buffer) >= 0
                 /\ UNCHANGED << Sent, Got, SenderLive, ReceiverLive, SenderState, Buffer, msg, ReceiverIT, NotifyRead, NotifyWrite, SenderIT >>
...
ReceiverRead ==  /\ ReceiverLive
                 ...
                 /\ \E k \in 1..Min(Len(Buffer),MaxReadLen):
                    /\ Len(Buffer) - k >= 0 (* On ne sort pas de la taille max *)
                    /\ Got' = Got \o Take(Buffer,k) (* On ajoute à l'historique *)
                    /\ Buffer' = Drop(Buffer,k) (* On retire au buffer *)
...
ReceiverReadNext == /\ ReceiverLive
                    /\ ReceiverState = AfterReading
                    /\ ReceiverState' = Idle
                    /\ ReceiverIT' = NotifyRead
                    /\ NotifyRead' = FALSE
                    /\ UNCHANGED << Sent, Got, SenderLive, ReceiverLive, SenderState, Buffer, msg, NotifyWrite, SenderIT >>
...
ReceiverUnblock == /\ ReceiverLive
                   ...
                   /\ ReceiverState = Blocked
                   /\ ReceiverState' = Idle
                   /\ UNCHANGED << Sent, Got, SenderLive, ReceiverLive, SenderState, Buffer, msg, ReceiverIT, NotifyRead, NotifyWrite >>


\* Transition any state -> Done. Receiver is no longer live.
ReceiverEnd == /\ \neg ReceiverLive 
               /\ UNCHANGED << Sent, Got, SenderLive, ReceiverLive, SenderState, ReceiverState, Buffer, msg, ReceiverIT, NotifyRead, NotifyWrite, SenderIT >>
