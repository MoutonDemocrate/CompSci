...
void parallel_scan(int *x, int b, int n){
  ...
  // Les tasks sont priorisées pour s'assurer qu'elles seront traitées dans le bon ordre une fois créées.
  ...
  #pragma omp master
  {
    ...
    for(d=0; d<b; d++){
      for(i=0; i<n; i+=1<<d+1){
        l = i+(1<<d)-1;
        r = i+(1<<(d+1))-1;
        #pragma omp task firstprivate(l,r) depend(in:x[l]) depend(inout:x[r]) priority(3)
        {
          x[r] = sum(x[l], x[r]);
        }
      }
    ...
    #pragma omp task depend(out:x[n-1]) priority(2)
    {
    x[n-1]=0;
    }
    ...
    for(d=b-1; d>=0; d--){
      for(i=0; i<n; i+=1<<(d+1)){
        l = i+(1<<d)-1;
        r = i+(1<<(d+1))-1;
        #pragma omp task firstprivate(l,r,t) depend(inout:x[l], x[r]) priority(1)
        {
        t = x[l];
        x[l] = x[r];
        x[r] = sum(t, x[l]);
