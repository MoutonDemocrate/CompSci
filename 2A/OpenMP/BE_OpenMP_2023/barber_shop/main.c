...
void barber_shop_par_fixed(int nbarbs, int nchairs){
  //declare locks for chairs
  omp_lock_t *locksChairs;
  locksChairs = (omp_lock_t*)malloc(nchairs*sizeof(omp_lock_t));
  int barber, client, chair;
  chair = 0;
#pragma omp parallel firstprivate(barber, client, chair) shared(locksChairs)
  {
    for(;;){
      ...
      chair = barber%nchairs;
      while (!omp_test_lock(&locksChairs[chair])) {
        //wait for chair to be free
      }
      serve_client(barber, client, chair);
      omp_unset_lock(&locksChairs[chair]);
    }
  }
}
