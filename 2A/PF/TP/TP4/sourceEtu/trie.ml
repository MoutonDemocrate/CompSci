...
let appartient mot trie =
  match trie with
  | Trie(arbre, fd, _) -> appartient_arbre (fd mot) arbre
...
let retrait mot trie =
  if appartient mot trie then
    match trie with
    | Trie(arbre,fd,fr) -> Trie(retrait_arbre (fd mot) arbre, fd, fr)
    else trie
...
let trie_dico trie =
  match trie with
  | (arbre,_,fr) -> List.map fr (liste_mots_arbre arbre)
