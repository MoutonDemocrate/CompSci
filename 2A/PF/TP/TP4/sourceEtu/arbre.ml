...
(******************************************************************************)
(*   liste les mots d'un arbre dans l'ordre lexicographique                   *)
(*   signature  : liste_mots_arbre : 'a arbre -> 'a list list                 *)
(*   paramètres :                                                             *)
(*       - un arbre n-aire                                                    *)
(*   résultat   : un liste de listes de charactères                           *)
(******************************************************************************)
let rec liste_mots_arbre arbre =
   let aux_branche (a,br) =
      List.map (fun el -> a::el) (liste_mots_arbre br)
   in
   match arbre with
   | Noeud(b,lb) -> let full = List.flatten (List.map aux_branche lb)
                    in if b then []::full else full
