...
and parse_I : (char, inst) parser = fun flux ->
  (
    (return []) ++
    (parse_C >>= fun c -> p_ptvirg
             >>= fun _ -> parse_I
             >>= fun i -> return (c::i))
  ) flux
and parse_C : (char, cmd) parser = fun flux ->
  (
     (p_repeat >>= fun _ -> p_entier
               >>= fun n -> parse_P
               >>= fun p -> return (Repeat (n, p))) ++
    (p_move >>= fun _ -> p_entier 
            >>= fun n -> return (Move (n))) ++ 
    (p_turn >>= fun _ -> p_entier 
            >>= fun n -> return (Turn (n))) ++ 
...
let rec run_flux_prog f =
  match Solution.uncons f with
  | None       -> failwith("Parsing error")
  | Some(p, q) -> run_logo p ; run_flux_prog q

let () = 
  flux_of_file "test.LOGO" |> parse_logo |> run_flux_prog