...
let rec insert ordre elt l =
  (* ordre a b = true si a>=b, false sinon*)
  match l with
    | [] -> [elt]
    | t::q -> 
      if ordre elt t then elt::t::q 
...
*)
let listStat = 
  let input = open_in "/home/lae/Documents/Work/CompSci/2A/PF/TP/TP2/nat2016.txt" in 
...
*)
let listStatHomme = 
  let input = open_in "/home/lae/Documents/Work/CompSci/2A/PF/TP/TP2/nathomme2016.txt" in
  let filebuf = Lexing.from_channel input in
  Parser.main Lexer.token filebuf
...
let rec scinde_v2 l = 
  (let rec scinde_v2_aux l acc len =
    if len <= 0 then
        l,acc
    else
        match l with
        | [] -> [],acc
        | t::q -> scinde_v2_aux q (t::acc) (len-1)

    ) in scinde_v2_aux l [] ((List.length l)/2)

(* TESTS *)
(* Peuvent être modifiés selon l'algorithme choisi *)
let%test _ = scinde_v2 [1;2;3;4] = ([1;3],[2;4])
let%test _ = scinde_v2 [1;2;3] = ([1;3],[2])
let%test _ = scinde_v2 [1] = ([1],[])
let%test _ = scinde_v2 [] = ([],[])

let fusionne_v2 ordre l1 l2 =
  let rec fusionne_v2_aux l1 l2 acc =
      match l1,l2 with
      | l1,[] -> (List.rev l1)@acc
      | [],l2 -> (List.rev l2)@acc
      | t::q, u::r -> 
          if ordre t u then
            fusionne_v2_aux q l2 (t::acc)
          else
            fusionne_v2_aux l1 r (u::acc)
  in
  fusionne_v2_aux l1 l2 []


(*TESTS*)
let%test _ = fusionne_v2 (fun x y -> x<y) [1;2;4;5;6] [3;4] = [1;2;3;4;4;5;6]
let%test _ = fusionne_v2 (fun x y -> x<y) [1;2;4] [3;4] = [1;2;3;4;4]
let%test _ = fusionne_v2 (fun x y -> x<y) [1;2;4] [3;4;8;9;10] = [1;2;3;4;4;8;9;10]
let%test _ = fusionne_v2 (fun x y -> x<y) [] [] = []
let%test _ = fusionne_v2 (fun x y -> x<y) [1] [] = [1]
let%test _ = fusionne_v2 (fun x y -> x<y) [] [1] = [1]
let%test _ = fusionne_v2 (fun x y -> x<y) [1] [2] = [1;2]
let%test _ = fusionne_v2 (fun x y -> x>y) [1] [2] = [2;1]